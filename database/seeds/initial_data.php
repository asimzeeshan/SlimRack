 ...
 * Initial seed data
 *
 * Populates reference tables with default data
 */
return new class {
    public function run(Connection $db): void
    {
        $this->seedPaymentCycles($db);
        $this->seedCurrencies($db);
        $this->seedCountries($db);
    }

    private function seedPaymentCycles(Connection $db): void
    {
        $cycles = [
            ['name' => 'Monthly', 'month' => 1],
            ['name' => 'Quarterly', 'month' => 3],
            ['name' => 'Semi-Yearly', 'month' => 6],
            ['name' => 'Yearly', 'month' => 12],
            ['name' => 'Bi-Yearly', 'month' => 24],
            ['name' => 'Tri-Yearly', 'month' => 36],
        ];

        foreach ($cycles as $cycle) {
            // Check if already exists
            if (!$db->exists('payment_cycle', 'name = ?', [$cycle['name']])) {
                $db->insert('payment_cycle', $cycle);
            }
        }
    }

    private function seedCurrencies(Connection $db): void
    {
        // Rate is multiplied by 10000 for precision (1 USD = 10000)
        $currencies = [
            ['currency_code' => 'USD', 'rate' => 10000],
            ['currency_code' => 'EUR', 'rate' => 10800],
            ['currency_code' => 'GBP', 'rate' => 12600],
            ['currency_code' => 'MYR', 'rate' => 2200],
            ['currency_code' => 'JPY', 'rate' => 67],
            ['currency_code' => 'AUD', 'rate' => 6400],
        ];

        foreach ($currencies as $currency) {
            if (!$db->exists('currency_rate', 'currency_code = ?', [$currency['currency_code']])) {
                $db->insert('currency_rate', $currency);
            }
        }
    }

    private function seedCountries(Connection $db): void
    {
        $countries = [
            ...
            ['country_code' => 'GS', 'country_name' => 'South Georgia'],
        ...
        foreach ($countries as $country) {
            if (!$db->exists('country', 'country_code = ?', [$country['country_code']])) {
